...
int MainWindow::FileIDConverter( const QString& s_FilenameIn, QStringList& sl_FilenameOut, structStation *Station_ptr, const int i_NumOfFiles )
{
    int				i_Year			= 2000;
    int				i_Month			= 1;
    int				i_Day			= 1;
...
// **********************************************************************************************
    ...
    if ( existsFirstFile( gi_ActionNumber, gs_FilenameFormat, gi_Extension, gsl_FilenameList ) == true )
    {
        ...
        if ( err == _NOERROR_ )
            err = concatenateFiles( gs_Path + "/" + "BSRN_LR0001.txt", sl_FilenameOut, tr( "Building LR0001 file..." ), 1, true );
    }
    else
    {
        err = _CHOOSEABORTED_;
...
// **********************************************************************************************
    ...
    endTool( err, stopProgress, gi_ActionNumber, gs_FilenameFormat, gi_Extension, gsl_FilenameList, tr( "Done" ), tr( "File ID converter was canceled" ), false, false );
