                    ...
                    if ( b_Month[j] == true )
                    {
                        ...
                        for ( int k=1; k<=MAX_NUM_OF_YEARS; k++ )
                        {
                            if ( b_Year[k] == true )
                            {
                                ...
                                //write File names into stringList:
                                if ( ( b_DecompressFiles == true ) || ( b_CheckFiles == true ) ) {
    ...
    else
    {
        ...
        while ( ( i < MAX_NUM_OF_STATIONS ) && ( stopProgress != _APPBREAK_ ) )
        {
            if ( b_Station[i] == true )
            {
                ...
                QFileInfo fi( s_DownloadPath + s_EventLabel + "_filelist.txt");
...
// **********************************************************************************************
    ...
    if ( b_OK == true )
    {
        ...
        {
        case 0:
            s_Message.append( tr( "No file has been downloaded. Please also check your ftp account!" ) );
            break;
